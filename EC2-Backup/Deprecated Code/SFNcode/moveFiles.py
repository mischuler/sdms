...
This function is used for file movement. Movement includes:
- S3 to S3IA (infrequent access)
- S3IA to S3
- S3 or S3IA to Glacier Archive
- Glacier Archive to S3IA

Changing between S3 states only requires an API call to AWS.
Changing to/from archive state to S3 requires physically pulling the files down and re-uploading
